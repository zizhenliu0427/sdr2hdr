# sdr2hdr — RTX Video SDK HDR / VSR 转换工具（图形界面 + 命令行）

> **语言 / Language**: **中文** · [English](README.md)

基于 英伟达 RTX Video SDK 的 AI 视频增强工具，支持 RTX HDR（SDR 转 HDR）和 RTX VSR（超分辨率）。与浏览器或播放器中的实时滤镜不同，sdr2hdr 会处理并将增强后的视频导出，不受原本只能实时处理播放的限制。

它是**单一二进制**：双击 `sdr2hdr.exe` 打开 **WinUI 3 图形界面**（拖放队列、模式/编码器面板、实时 ETA、依赖检测），或给*同一个* exe 传命令行参数当 **CLI / 批处理**用（见[使用方式](#使用方式)）。命令行与图形界面共用同一套引擎。
...
**双击 `sdr2hdr.exe`** 打开 WinUI 3 窗口：

- **拖入视频**到拖放区（或点 *添加文件…*）；每行右侧有 ✕ 可删除该任务。
- 选**模式**（RTX HDR / RTX VSR / RTX VSR + HDR）——无关选项会自动灰掉。
- **编码器**：GPU (NVENC) 或 CPU (软件)。*RTX HDR/VSR 转换始终在 GPU 上运行*——这里只切换视频编码器（CPU = 软件 x265/x264/AV1）。
- 调整**编码 / 质量 / 峰值尼特 / 输出分辨率 / VSR 质量**（每项带 ⓘ 悬停说明）。
- **开始转换**——队列实时显示 `45%  127.0 fps  ~2:13`（百分比、fps、预估剩余），任务栏同步进度，完成弹系统通知。最后的音频合并阶段显示「合并音频中…」并带已用时间计时。
- 如果拖入的视频**本身已经是 HDR**，顶部会弹提示（sdr2hdr 是把 *SDR → HDR*）。
- **命令行**页：直接输入 CLI 参数在控制台运行。**设置**页：主题、语言（中/英）、默认输出目录，以及依赖检测（ffmpeg / NVNGX DLL）带官方下载链接。

> 同一个程序两副面孔：**不带参数**显示图形界面；**带参数**则与下面的 CLI 行为完全一致。

### 交互式控制台向导

图形界面版无参时显示窗口，所以要进**控制台向导**请加 `--interactive`（或用无界面的控制台构建，那个版本裸双击即进向导）：
...
> **状态：** **v1.1 WinUI 3 图形界面已交付**（`TODO.md` #33、#37–#45）——GUI 与 CLI 已合并为单一二进制。**v1.2 音画可靠性大部分交付**：音画同步 / VFR 修复 / 逐帧 PTS 透传（#32）已落在 GPU 主路径上，但字幕/章节直通、`--output-fps`、HDR10 SEI 仍未做。

### ✅ v1.1 — WinUI 3 图形界面（已交付）

基于 WinUI 3 / Windows App SDK 的原生 Windows GUI，已与 CLI 合并为单一 `sdr2hdr.exe`。

- [x] **项目脚手架** — WinUI 3 C++/WinRT 项目 + WinAppSDK，`sdr2hdr` 引擎作为共用静态库
- [x] **主窗口** — 拖放区、文件选择器、队列列表（每行 ✕ 可删除）
- [x] **模式选择器** — RTX HDR / VSR / VSR+HDR，配套实时参数面板；无关选项自动禁用；编码器 GPU(NVENC)/CPU(软件) 开关；编码 / 质量 / 峰值尼特 / 输出分辨率 / VSR 质量 各档位带 ⓘ 说明
- [x] **批处理队列** — 状态（等待/处理中/完成/失败）、进度条、实时 **ETA + fps**、任务栏进度、完成系统通知
- [x] **CLI + GUI 合并二进制** — 无参→GUI，带参→CLI；内置「命令行」页
- [x] **设置页面** — 默认输出目录、主题（系统/浅/深）、语言（中/英）、依赖检测（ffmpeg / NVNGX）带下载链接
- [x] **关于** — 版本、GitHub 链接、app 图标、随主题着色的 ffmpeg / 英伟达 logo
- [~] **实时预览** — *已放弃*：GPU 管线不吐中间帧、输出又是 HDR，做不到边转边预览成片；原源视频回放面板已移除
- [ ] **启动检查更新** — 尚未实现

### 🔧 v1.2 — 转码质量与可靠性（部分交付）

#### 已交付（GPU-only 默认路径，2026-06）

- [x] **修复音画不同步（主路径）** — `probeVideo` 检测 VFR / 虚高 fps，实测 fps 驱动 NVENC/muxer，两阶段 mux（raw 视频 → `remuxCopyAudio`），MKV 音频交错修复，mux 后 `verifyMuxOutput` 自检。已测：PUBG Game DVR VFR MP4、SVP/OBS MKV。
- [x] **真 VFR PTS 透传（#32）** — 编码的同时并行采集源视频逐帧时间戳（packet 级 ffprobe），写进输出的 sample table（`mp4_retime.cpp` 原地重写 stts box）。平均 fps CFR 只能保证首尾对齐——20 分钟 VFR 录像中段实测漂移高达 **±10.6 秒**；透传后逐帧漂移 **0 ms**。任何异常自动回退 CFR 并告警；`--no-vfr-pts` 可关闭。

#### v1.2 计划

- [ ] **多进程并行转换（#46）** — v1.2 主打更新。单进程被 NVIDIA「每进程一个 NGX 实例」的硬限制卡在 ~125 fps @ 4K（GPU 整体利用率才 ~33%），把输入切成 N 段交给 N 个工作进程并行转换是唯一的提速路径（2 进程预期 ~2×）。工作进程模型已经落地（GUI 现在就是通过 `sdr2hdr_cli.exe` 跑转换做崩溃隔离的）。
- [ ] **项目瘦身（#49）** — 仓库内容已超出定位。计划:删除 `--legacy` 像素管道旧路径(GPU 路径是其稳定超集)、归档压缩 TODO 已完成项、两份 README 砍半。GUI 是唯一交付物,CLI 仅作内部 worker 与排障入口。

#### 其余小项
- [ ] **输出帧率（`--output-fps`）** — 编码阶段丢帧降 fps（如 250→120），非慢放（#34）
- [ ] **字幕轨直通** — `-map 0:s?`，ASS/SRT/PGS 在 remux 后保留
- [ ] **章节元数据复制** — `-map_metadata 0` 保留章节、标题与标签
- [ ] **HDR10 静态元数据 SEI** — 比特流内严格 SMPTE 2086 / CEA-861.3 SEI（当前：GPU 路径为 NVENC VUI + 容器 colour tag；legacy ffmpeg 路径在支持时有 `-master_display` / `-max_cll`）
...
**状态（2026-06）：** **GPU-only 默认路径**（不加 `--legacy`）已包含下列问题的修复。请尽量从**原片**重新转换；没有原片、只有旧版损坏导出时，需手动 ffmpeg remux（时间戳偏移 / 裁切），**不在**转换器自动修复范围内。

实现细节见 `TODO.md` **#17–#31**。

### 曾经的原因

**帧率不匹配** — VFR 源（PUBG DVR、OBS）与虚高元数据 fps（SVP 标 125、实测约 120）导致视频按错误 CFR 打时间戳，音频原样 copy → 整段加速或漂移。

**Mux 可靠性** — Windows 下 ffmpeg pipe + 双输入 remux 可能丢音轨（MP4）或 Opus 堆在文件末尾（MKV → 开头无声、拖进度条才正常）。

**重编码 vs 音频 copy** — NVENC 产生新视频时间线；fps / 帧数与源不一致时，音画不再共用同一时钟。

### 已修复（GPU-only 路径）

| 修复 | 作用 |
|------|------|
| `probeVideo` VFR + 实测 fps | 用真实平均 fps，不用错误的 `r_frame_rate` |
| 两阶段 mux | 编码 raw ES → `remuxCopyAudio`，避免 pipe 双输入 |
| ES → 纯视频容器 → 再合并 | 音频正确交错 |
| mux 后 `verifyMuxOutput` | 起始/结尾偏差或 EOF 音频堆叠时立即失败 |
| 逐帧 VFR PTS 透传（#32） | 平均 fps CFR 在长 VFR 素材中段漂移（20 分钟 DVR 实测 7 分钟处差 ±10.6 秒——开头结尾正常、拖到中间就不同步）；现输出携带源的精确逐帧时间戳 |

### 仍待处理

- **`--legacy` 管线** — 同类 mux 问题可能仍在（#29）
- **抢救旧损坏成品** — 仅手动 ffmpeg；`sdr2hdr` 不内置自动修复

### 若仍不同步

1. 非调试不要用 **`--legacy`**，优先默认 GPU-only 路径。
2. 从**原始录制**重转，不要从已损坏的导出再转。
3. 加 **`--verbose`**，查看 mux 后是否有 `verifyMuxOutput` 报错。
4. 最后手段（保留源音频）：
...
**降低输出帧率**（如 250→120 fps、非慢放）若针对已编码文件，需 ffmpeg `fps=120` 重编码；计划以转换时 **`--output-fps`** 提供（#34）。
